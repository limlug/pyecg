...
class ThreadSample(QtCore.QThread):
    ...
    def __init__(self, parent=None):
        ...
        #self.sam=np.zeros( (1000,2), dtype=np.int16 )
        ...
        if self.ser.inWaiting() > 0:
            ...
            for i in range(len(read_byte)):
                if binascii.hexlify(read_byte[i])=="ff":
                    ...
                    #out2 = fields[2] << 8
                    #out2 += fields[3]
                    if out > 1024: #or out2 > 5000:
                        break
                    self.sam[0][self.pos]=out
                    #self.sam[1][self.pos]=out2
                    if(self.pos==1999):
...
class MyWindow(QtGui.QWidget):
    ...
    def init_thread(self):
        ...
        #self.line3, = self.matplotlibWidget.axis1.plot(self.movingaverage(self.threadSample.sam[0], 10), '-', alpha=0.8, color="blue", markerfacecolor="blue")
        # Initialisiere Timer
        self.qrs=[]
        self.dra=True
        self.qrs_f=[]
    ...
    def smooth(self,x,window_len=11,window='blackman'):
        ...
        y=np.convolve(w/w.sum(),s,mode='valid') #war 'valid'
        return y
    ...
    def on_threadSample_newSample(self, sample, use_blit=True):
        ...
        self.line1.set_ydata((z-512)*0.002929688)
            #self.line3.set_ydata((self.movingaverage(z,10)-512)*0.002929688)
